...
fn allocToResponse(allocator: std.mem.Allocator, id: i64, result: anytype) []const u8 {
    var buf = std.ArrayList(u8).init(allocator);
...

    {
        ...
        if (!isNull(result)) {
            ...
            // w.stringify(result) catch unreachable;
...
fn logJson(allocator: std.mem.ArenaAllocator, json: ?std.json.Value) void {
...
fn logT(allocator: std.mem.Allocator, value: anytype) void {
...
pub fn initialize(self: *Self, arena: *std.heap.ArenaAllocator, id: i64, jsonParams: ?std.json.Value) ![]const u8 {
    ...
    return allocToResponse(arena.allocator(), id, lsp.initialize.InitializeResult{
...
pub fn shutdown(self: *Self, arena: *std.heap.ArenaAllocator, id: i64, jsonParams: ?std.json.Value) ![]const u8 {
    ...
    return allocToResponse(arena.allocator(), id, null);
}
